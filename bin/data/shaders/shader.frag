...
void main()
{
    ...
    float r = sin(iGlobalTime) * 0.5;
    float g = (gl_FragCoord.x / iResolution.x);
    float b = gl_FragCoord.y / iResolution.y - sin(iGlobalTime); //sin(iGlobalTime);
    float a = shapeOpacity;
...
//	}
//	
//    vec4 finalResult = vec4(sqrt(vResult),1.0);
//    
//    float finalR = sin(iGlobalTime) * 0.5;
//    float finalG = (gl_FragCoord.x / iResolution.x);
//    float finalB = gl_FragCoord.y / iResolution.y - sin(iGlobalTime); //sin(iGlobalTime);
//    float finalA = shapeOpacity*finalResult.r;
//    
//	gl_FragColor = vec4(finalR, finalG, finalB, finalA);
//}